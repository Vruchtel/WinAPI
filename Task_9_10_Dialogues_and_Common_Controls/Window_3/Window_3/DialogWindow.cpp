...
bool CDialogWindow::Create(HWND hWndParent)
{
	...
	CreateDialogParam(GetModuleHandle(0), MAKEINTRESOURCE(IDD_DIALOG1), hWndParent, dialogProc,
...
void CDialogWindow::OnInit(HWND _handle)
{
	...
	// получим у edit-control window и overlapped window текущие свойства
	LOGFONT logFont;
	...
	SendMessage(GetDlgItem(handle, IDC_TRANSPARENCY), TBM_SETPOS, true, newSettings.transparency);
...
/*void CDialogWindow::OnDestroy(WPARAM wParam, LPARAM lParam)
{
	EndDialog(handle, wParam);
}*/
